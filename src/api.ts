...
export function getSearchMovieData(queryData: string) {
  return fetch(
    `${BASE_PATH}/search/movie?api_key=${API_KEY}&query=${queryData}&language=ko-KR&region=kr`
  ).then((response) => response.json());
}

export function getSearchTvData(queryData: string) {
  return fetch(
    `${BASE_PATH}/search/tv?api_key=${API_KEY}&query=${queryData}&language=ko-KR&region=kr`
  ).then((response) => response.json());
}
