...
import { getTopTv, IGetTvsResult } from "../api";
...
const TopTvSlider = () => {
  ...
  const bigTvMatch = useMatch("/tv/:tvId/top");
  ...
  const { data: topTvData, isLoading } = useQuery<IGetTvsResult>(
    ["topTv", "top"],
    getTopTv
  );
  ...
  const onBoxClicked = (tvId: number) => {
    navigate(`/tv/${tvId}/top`);
  };
  const onOverlayClick = () => navigate("/tv");
  const clickedTv =
    bigTvMatch?.params.tvId &&
    topTvData?.results.find(
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <SliderWrapper>
            <SliderTitle>평정높은 프로그램</SliderTitle>
            <Slider>
