...
import { getSearchMovieData, IGetMoviesResult } from "../api";
...
const SearchMovieSlider = () => {
  const navigate = useNavigate();
  const location = useLocation();

  const queryString = new URLSearchParams(location.search).get("keyword");

  const bigMovieMatch = useMatch(`/search?keyword=${queryString}/:movieId`);
  ...
  const { data, isLoading } = useQuery<IGetMoviesResult>(
    ["movies", queryString],
    () => getSearchMovieData(queryString ? queryString : "")
  );
  ...
  const onBoxClicked = (movieId: number) => {
    navigate(`/search?keyword=${queryString}/${movieId}`);
  };
