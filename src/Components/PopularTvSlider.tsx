...
import { getPopularTv, IGetTvsResult } from "../api";
...
const PopularTvSlider = () => {
  ...
  const bigTvMatch = useMatch("/tv/:tvId/pop");
  ...
  const { data: popularTvData, isLoading } = useQuery<IGetTvsResult>(
    ["popularTv", "popular"],
    getPopularTv
  );
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <SliderWrapper>
            <SliderTitle>인기있는 티비 프로그램</SliderTitle>
            <Slider>
          ...
          <AnimatePresence>
            {bigTvMatch ? (
              <>
                ...
                />
                <BigMovie
                  layoutId={`${bigTvMatch.params.tvId}pop`}
